...
void Stadium::DrawMat(){
	...
	for (int i = 0; i<=(compSize + boundarySize +2)/2; i++){
		for (int j = 0; j<=(compSize + boundarySize+4)/2; j++){
			if (abs(i) > compSize/2 + 1 or abs(j) > compSize/2 +1){
				glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, static_cast<GLfloat*>(wDiffuse));
			}
			else if (abs(i) > compSize/2 or abs(j) > compSize/2){
